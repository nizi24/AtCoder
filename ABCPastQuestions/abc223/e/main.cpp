...
ll tate(ll left, ll right, ll A, ll Y) {
    ll ng = left;
    ll ok = right;
...
void solve(long long X, long long Y, long long A, long long B, long long C) {
    ...
    do {
        // 縦
        ll left = tate(0, X, abc[0], Y);

        ll nokori = X - left;

        // 縦、縦
        if ((nokori - myceil(abc[1], Y)) * Y >= abc[2]) {
            c(YES);
            return;
        }

        // 縦、横
        if (nokori > 0 && (Y - myceil(abc[1], nokori)) * nokori >= abc[2]) {
            c(YES);
            return;
        }

        // 横
        ll up = tate(0, Y, abc[0], X);

        nokori = Y - up;

        // 横、横
        if ((nokori - myceil(abc[1], X)) * X >= abc[2]) {
            c(YES);
            return;
        }

        // 横、縦
        if (nokori > 0 && (X - myceil(abc[1], nokori)) * nokori >= abc[2]) {
            c(YES);
            return;
        }
