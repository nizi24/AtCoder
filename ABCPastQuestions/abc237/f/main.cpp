...
void solve(long long N, long long M) {
    vector<vector<vector<vector<mint>>>> dp(N+1,
        vector<vector<vector<mint>>>(M+1, vector<vector<mint>>(M+1, vector<mint>(M+1, 0))));
    ...
    for (int i = 0; i < N; i++) {
        REP (x, M) {
            REP (a, M+1) REP (b, M+1) REP (c, M+1) {
                ...
                if (x <= a) dp[i + 1][x][b][c] += n;
                else if (x <= b) dp[i + 1][a][x][c] += n;
                else if (x <= c) dp[i + 1][a][b][x] += n;
            }
        }
    }
...
const int MOD = 998244353; // 問題ごとに変更する
