...
void solve(long long N, long long M, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C, std::vector<long long> D) {
    ...
    REP (i, N) event.push_back({{A[i], B[i]}, 0});
    REP (i, M) event.push_back({{C[i], D[i]}, 1});
    sort(all(event));

    multiset<ll> chocolate;
    for (auto e : event) {
        if (e.second) {
            auto iter = chocolate.upper_bound(e.first.second);
            if (iter == chocolate.begin()) continue;
            iter--;
            chocolate.erase(iter);
        } else {
            chocolate.insert(e.first.second);
        }
    }

    c(chocolate.empty() ? YES : NO);
}
