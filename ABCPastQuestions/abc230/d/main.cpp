...
void solve(long long N, long long D, std::vector<long long> L, std::vector<long long> R) {
    vector<pair<ll, pair<ll, ll>>> event;
    REP (i, N) {
        event.push_back({L[i], {0, i}});
        event.push_back({R[i]+D-1, {1, i}});
    }
    R.push_back(INF64);
    ...
    REP (i, 2 * N) {
        ll z = event[i].first;
        ll a = event[i].second.first;
        ll j = event[i].second.second;
        if (!a) {
            mada.push_back(j);
        } else {
            if (!st.count(j)) {
                ans++;
                for (auto m : mada) st.insert(m);
                mada.clear();
            }
        }
    }
...
int main(){
    ...
    solve(N, D, std::move(L), std::move(R));
    return 0;
}
