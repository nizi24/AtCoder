...
multiset<ll> dfs(int n, int p, vector<vector<ll>> &G, vector<vector<ll>> &q, vector<ll> &X) {

    multiset<ll> st;
    ll sz = 0;
    for (auto a : G[n]) {
        if (a != p) { // 親への逆流防止
           auto child_st = dfs(a, n, G, q, X);
           REP (i, 20) {
               if (child_st.empty()) break;
                auto t = *child_st.rbegin();
                child_st.erase(child_st.find(t));
               if (sz <= 20) {
                   st.insert(t);
                   sz++;
               } else if (t > *st.begin()) {
...
void solve(long long N, long long Q, std::vector<long long> X, std::vector<long long> A, std::vector<long long> B, std::vector<long long> V, std::vector<long long> K) {
    ...
    vector<vector<ll>> q(N, vector<ll>(20, 0));
    multiset<ll> st;
    dfs(0, -1, G, q, X);

    REP (i, Q) {
        cout << q[V[i]-1][K[i]-1] << endl;
    }
}
