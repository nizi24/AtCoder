...
void push(ll x, UnionFind &Uf, set<ll> &box) {
    Uf.push(x);
    box.insert(x);
    if (box.count(x-1)) {
        Uf.unite(x-1, x);
    ...
    REP (i, T) {
        ...
        REP (i, N) {
            ...
            if (!box.count(l)) {
                ...
                if (Uf.root(l)+1 > r) ans = NO;
                else push(Uf.root(l)+1, Uf, box);
            }
...
// UnionFind構造体
struct UnionFind {
    map<long long, long long> par, siz;
