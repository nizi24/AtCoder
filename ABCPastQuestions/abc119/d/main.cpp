...
void solve(long long A, long long B, long long Q, std::vector<long long> s, std::vector<long long> t, std::vector<long long> x) {
    ...
    REP (i, Q) {
        auto ls = abs(x[i] - *(upper_bound(all(s), x[i]) - 1));
        auto lt = abs(x[i] - *(upper_bound(all(t), x[i]) - 1));
        auto rs = abs(x[i] - *(upper_bound(all(s), x[i])));
        auto rt = abs(x[i] - *(upper_bound(all(t), x[i])));

        ll ans = min(max(ls, lt), max(rs, rt));
        chmin(ans, min(ls * 2 + rt, lt * 2 + rs));
        chmin(ans, min(ls + rt * 2, lt + rs * 2));
