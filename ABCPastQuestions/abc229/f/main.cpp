...
void solve(long long N, std::vector<long long> A, std::vector<long long> B) {
    vector<vector<vector<ll>>> dp(N+1, vector<vector<ll>>(2, vector<ll>(2, INF64)));
    dp[1][0][1] = 0;
    dp[1][0][0] = A[0];
    ...
    for (int i = 2; i <= N; i++) {
        REP (j, 2) REP (k, 2) {
            ll m = (j == k ? A[i-1] : 0);
            ll c = (i == N ? (j ? 0 : B[i-1]) : 0);
            chmin(dp[i][j][k], dp[i-1][1-j][k] + m + c);
            chmin(dp[i][j][k], dp[i-1][j][k] + m + B[i-2] + c);
        }
    }

    ll ans = INF64;
    REP (j, 2) REP (k, 2) chmin(ans, dp[N][j][k]);
    c(ans);
}
