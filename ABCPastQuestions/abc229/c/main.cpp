...
void solve(long long N, long long W, std::vector<long long> A, std::vector<long long> B) {
    vector<pair<ll, ll>> vec;
    REP (i, N) vec.push_back({A[i], B[i]});
    sort(rall(vec));

    ll ans = 0;
    REP (i, N) {
        ans += min(W, vec[i].second) * vec[i].first;
        W -= vec[i].second;
        if (W < 0) break;
    }
    c(ans);
...
int main(){
    ...
    solve(N, W, std::move(A), std::move(B));
    return 0;
}
