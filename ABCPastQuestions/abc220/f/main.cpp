...
void dfs(int n, int p, vector<vint> &G, vector<ll> &child) {
    child[n]++;
...
void dfs2(int n, int p, vector<ll> &ans, vector<ll> &child, ll N, vector<vint> &G) {
    if (p != -1) {
        ans[n] = ans[p] - child[n] + (N - child[n]);
    }

    for (auto a : G[n]) {
        if (a != p) { // 親への逆流防止
           dfs2(a, n, ans, child, N, G);
       }
    }
}
