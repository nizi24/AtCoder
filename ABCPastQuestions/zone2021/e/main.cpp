...
void solve(long long R, long long C, std::vector<std::vector<long long>> A, std::vector<std::vector<long long>> B) {
    vector<vector<ll>> dist(R, vector<ll>(C, INF64));
    ...
    while (!que.empty()) {
        ...
        // (r, c+1)
        if (c < C-1 && chmin(dist[r][c+1], d + A[r][c])) {
            que.push({dist[r][c+1], {r, c+1}});
        }

        // (r, c-1)
        if (c > 0 && chmin(dist[r][c-1], d + A[r][c-1])) {
            que.push({dist[r][c-1], {r, c-1}});
        }

        // (r+1, c)
        if (r < R-1 && chmin(dist[r+1][c], d + B[r][c])) {
            que.push({dist[r+1][c], {r+1, c}});
        }

        // (r-i, c)
        for (int nr = r-1; nr >= 0; nr--) {
            if (chmin(dist[nr][c], d + 1+(r-nr))) {
                que.push({dist[nr][c], {nr, c}});
            }
        }
    }
...
struct Edge {
    pair<int, int> to; // 隣接頂点番号
    long long w; // 重み
    Edge(pair<int, int> to, long long w) : to(to), w(w) {}
};
