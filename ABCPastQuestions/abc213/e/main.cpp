...
int main(){
    ...
    dist[0][0] = 0;
    priority_queue<tuple<ll, pair<ll, ll>, bool>, vector<tuple<ll, pair<ll, ll>, bool>>, greater<tuple<ll, pair<ll, ll>, bool>>> que;
    que.push({0, {0, 0}, 0});

    while (!que.empty()) {
        ll d = get<0>(que.top());
		ll y = get<1>(que.top()).first;
		ll x = get<1>(que.top()).second;
        bool flag = get<2>(que.top());
		que.pop();
        ...
        if (flag) {
            for (int i = -1; i <= 1; i++) {
                for (int j = -1; j <= 1; j++) {
                    if (y + i >= H || y + i < 0 || x + j >= W || x + j < 0) continue;

                    if (chmin(dist[y + i][x + j], d)) {
                        que.push({d, {y + i, x + j}, 0});
                    }
                }
            }
        } else {
            ...
            for (int i = -1; i <= 1; i += 2) {
                if (y + i < H && y + i >= 0 && S[y + i][x] == '#') {
                    if (chmin(dist[y + i][x], d+1)) {
                        que.push({d+1, {y + i, x}, 1});
                    }
                }
                if (x + i < W && x + i >= 0 && S[y][x + i] == '#') {
                    if (chmin(dist[y][x + i], d+1)) {
                        que.push({d+1, {y, x + i}, 1});
                    }
                }
            }
        }
