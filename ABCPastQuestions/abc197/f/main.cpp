...
int main(){
    ...
    vector<map<char, vector<ll>>> G(N);
    REP (i, M) {
        G[A[i]-1][C[i]].push_back(B[i]-1);
        if (A[i] != B[i]) G[B[i]-1][C[i]].push_back(A[i]-1);
    }
    ...
    while (!que.empty()) {
        ...
        for (auto g : G[p.first]) {
            for (auto nx1 : g.second) {
                if (nx1 == p.second) {
                    c(d + 1)
                    return 0;
                }
        ...
        for (auto g : G[p.first]) {
            for (auto nx1 : g.second) {
                for (auto nx2 : G[p.second][g.first]) {
                    if (nx1 == nx2) {
                        c(d + 2)
                        return 0;
                    }
