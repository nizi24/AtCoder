...
int main(){
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (n > 0) {
            chmin(dp[i + 1][0], dp[i][0] + abs(pre0 - X[i][0]) + abs(X[i][0] - X[i][n - 1]));
            chmin(dp[i + 1][0], dp[i][1] + abs(pre1 - X[i][0]) + abs(X[i][0] - X[i][n - 1]));

            chmin(dp[i + 1][1], dp[i][0] + abs(pre0 - X[i][n - 1]) + abs(X[i][0] - X[i][n - 1]));
            chmin(dp[i + 1][1], dp[i][1] + abs(pre1 - X[i][n - 1]) + abs(X[i][0] - X[i][n - 1]));
    ...
    c(min(dp[N][0] + abs(pre0), dp[N][1] + abs(pre1)))
