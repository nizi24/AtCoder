...
int main(){
    ...
    vector<vector<pair<ll, ll>>> q(N+1);
    REP (i, M) q[X[i]].push_back({Y[i], Z[i]});

    REP (bit, 1 << N) {
        vector<ll> v;
        REP (i, N) if (bit & (1 << i)) v.push_back(i+1);

        bool flag = 0;
        ll sz = v.size();
        for (auto x : q[sz]) {
            if (upper_bound(all(v), x.first) - v.begin() > x.second) {
                flag = 1;
                break;
            }
        }
