...
void solve(long long N, long long M, long long K, std::vector<long long> A, std::vector<long long> U, std::vector<long long> V) {
    vector<vector<ll>> G(N);
    map<pair<int, int>, int> edge;
    REP (i, N-1) {
        G[U[i]-1].push_back(V[i]-1);
        G[V[i]-1].push_back(U[i]-1);
        edge[{min(U[i], V[i])-1, max(U[i], V[i])-1}] = 0;
    }

    REP (i, M-1) {
        int r = A[i]-1; // 始点
        vector<long long> route(N, -1);
        queue<long long> que;
        que.push(r);
        route[r] = INF;
        ...
        int nx = route[cur];
        while (nx != INF) {
            edge[{min(cur, nx), max(cur, nx)}]++;
            cur = nx;
            nx = route[cur];
        }
    }

    vector<mint> cur(200001, 0), nx(200001, 0);
    cur[100000] = 1;
    ...
    for (auto m : edge) {
        REP (j, 200001) {
            ...
            if (j + m.second <= 200000) nx[j + m.second] += cur[j];
            if (j - m.second >= 0) nx[j - m.second] += cur[j];
        }
...
int main(){
    ...
    solve(N, M, K, std::move(A), std::move(U), std::move(V));
    return 0;
}
