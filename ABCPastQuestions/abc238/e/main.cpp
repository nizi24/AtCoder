...
struct RMQ_RAQ_LazySegTree {
    int n; // 葉の数
    vector<long long> node; // 完全2分木
    vector<long long> lazy; // 遅延配列

    // 初期化
    RMQ_RAQ_LazySegTree(vector<long long> v) {
        int sz = v.size();
        n = 1;
...
struct RMQ_RAQ_LazySegTree2 {
    int n; // 葉の数
    vector<long long> node; // 完全2分木
    vector<long long> lazy; // 遅延配列

    // 初期化
    RMQ_RAQ_LazySegTree2(vector<long long> v) {
        int sz = v.size();
        n = 1;
        while (n < sz) n *= 2; // 与えられた数列の項数以上の2^n個、葉を作る
        node.resize(n*2-1,  0); // 0で初期化
        ...
        for (int i = 0; i < sz; i++) node[n-1+i] = v[i]; // 葉を初期化
        for (int i = n-2; i >= 0; i--) node[i] = max(node[i*2+1], node[i*2+2]); // 下から順に葉以外のnodeを初期化
    }
...
int main(){
    ...
    UnionFind Uf(N+1);
    REP (i, Q) Uf.unite(l[i]-1, r[i]);

    if (Uf.issame(0, N)) c(YES);
    else c(NO);
