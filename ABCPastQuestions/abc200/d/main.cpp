...
void solve(long long N, std::vector<long long> A) {
    ...
    vector<vector<ll>> dp(210, vector<ll>(21000, 0));
    dp[0][0] = 1;
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j <= 21000; ++j) {
            (dp[i+1][j] += dp[i][j]) %= 200;
            if (j >= A[i]) (dp[i+1][j] += dp[i][j-A[i]]) %= 200;
        }
    }

    vector<vector<deque<int>>> ans(210);
    for (int k = 1; k < 20010; k++) {

        deque<int> keiro;
        vector<deque<int>> a;
        rec(dp, A, N, k, keiro, a);

        bool ok = 0;
        REP (i, a.size()) {
            ans[k % 200].push_back(a[i]);
