...
void solve(long long N, long long C, std::vector<long long> a, std::vector<long long> b, std::vector<long long> x) {
    // start = 0, end = 1
    vector<tuple<ll, ll, ll>> edge;
    REP (i, N) {
        edge.push_back({a[i], 0, i});
        edge.push_back({b[i] + 1, 1, i});
    }
    ALL(sort, edge);
    ...
    REP (i, N * 2 - 1) {
        ...
        ll next = get<0>(edge[i + 1]);
        ll current = get<0>(edge[i]);
        if (cur > C) ans += C * (next - current);
        else ans += cur * (next - current);
    }
...
int main(){
    ...
    solve(N, C, std::move(a), std::move(b), std::move(x));
    return 0;
}
