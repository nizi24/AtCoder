...
void solve() {
    ...
    foreach_permutation(10, N, [](int *indexes) {
        map<char, int> mp;
        int cnt = 0;
        ...
        REP (i, 3) {
            ...
            for (int j = 0; j < S[i].size(); j++) {
                if (mp.count(S[i][j])) tmp.push_back('0' + mp[S[i][j]]);
                else {
                    mp[S[i][j]] = indexes[cnt];
                    tmp.push_back('0' + n[indexes[cnt]]);
                    cnt++;
                }
            }
            res.push_back(tmp);
        ...
        if (res[0][0] != '0' && res[1][0] != '0' && res[2][0] != '0' &&
            stoll(res[0]) + stoll(res[1]) == stoll(res[2])) {
            ans[0] = res[0];
            ans[1] = res[1];
            ans[2] = res[2];
        }
