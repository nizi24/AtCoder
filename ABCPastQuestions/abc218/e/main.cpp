...
void solve(long long N, long long M, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C) {
    ...
    REP (i, M) vec.push_back({C[i], {A[i]-1, B[i]-1}});
    ALL(sort, vec);

    UnionFind Uf(N+1);
    ll ans = 0;
    REP (i, M) {
        if (Uf.issame(vec[i].second.first, vec[i].second.second)) {
            ans += max(0LL, vec[i].first);
        } else Uf.unite(vec[i].second.first, vec[i].second.second);
    }
