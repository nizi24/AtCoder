...
void solve(long long N, long long M, std::vector<long long> s, std::vector<long long> t) {
    ...
    REP (i, M) {
        if (saitan.count(i)) {
            ...
            REP (j, M) {
                if (i != j) G2[s[j]-1].push_back(t[j]-1);
            }
            int r = 0; // 始点
            vector<long long> dist2(N, INF64);
            queue<long long> que;
            que.push(r);
            dist2[r] = 0;
            ...
            c((dist2[N-1] == INF64 ? -1 : dist2[N-1]));
        } else {
            c((dist[N-1] == INF64 ? -1 : dist[N-1]));
        }
    }
