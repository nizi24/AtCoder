...
int rec(int y, int x) {
    ...
    if (memo[y][x] != INF) return memo[y][x];

    int res = -INF;
    if (y+1 < H) chmax(res, -rec(y+1, x) + (A[y+1][x] == '-' ? -1 : 1));
    if (x+1 < W) chmax(res, -rec(y, x+1) + (A[y][x+1] == '-' ? -1 : 1));
    return memo[y][x] = res;
}
...
int main(){
    cin >> H >> W;
    A.resize(H);
    memo.resize(H, vector<ll>(W, INF));
    REP (i, H) cin >> A[i];

    memo[0][0] = rec(0, 0);
    if (memo[0][0] > 0) c("Takahashi")
    else if (memo[0][0] < 0) c("Aoki")
    else c("Draw")
