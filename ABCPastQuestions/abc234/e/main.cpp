...
void dfs(ll x, ll d, vector<ll> &vec, ll cnt) {
    if (cnt >= 1) vec.push_back(x);
    if (x >= pw(10, 17)) return;

    ll nx = (x % 10) + d;
    if (nx < 10 && nx >= 0) dfs(x * 10 + nx, d, vec, cnt+1);
}

void solve(long long X) {
    vector<ll> vec;
    REP (i, 9) vec.push_back(i);
    REP (i, 9) REP (j, 10) dfs(i+1, j, vec, 0);
    REP (i, 9) REP (j, 10) dfs(i+1, -j, vec, 0);
