...
struct SegTree {
    ...
    SegTree(vector<pair<ll, ll>> v) {
        ...
        for (int i = 0; i < sz; i++) node[n-1+i] = modpow((mint)2, v[i].second-1); // 葉を初期化
    ...
    void find(int s, int t, mint &ans) { find_query(s, t, 0, n, 0, ans); }

    void find_query(int s, int t, int l, int r, int n, mint &ans) {
    ...
    // デバック用
    void output() {
        for (int i = n-1; i < n*2-1; i++) cout << node[i] << " ";
    }
...
void solve(long long N, std::vector<long long> A) {
    ...
    auto segtree = SegTree(w);

    mint ans = 0;
    REP (i, N-1) {
        segtree.update(mp[i], 0);

        ll iter = lower_bound(all(v), A[i]) - v.begin();

        mint cur = 0;
        segtree.find(iter, N, cur);
        ans += cur / modpow((mint)2, i);
    }
